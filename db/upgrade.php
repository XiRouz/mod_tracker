...
function xmldb_tracker_upgrade($oldversion = 0) {
    ...
    // SAMSUNG BEGIN - add reporter context id, possible context levels - StudentGroup, Faculty, Course
    if ($oldversion < 2023032800) {
        $table = new xmldb_table('tracker_issue');
        $field = new xmldb_field('reportercontextid', XMLDB_TYPE_INTEGER, 10, XMLDB_UNSIGNED, XMLDB_NOTNULL, null, 0, 'trackerid');
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }

        upgrade_mod_savepoint(true, 2023032800, 'tracker');
    }
    // SAMSUNG END
