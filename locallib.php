...
use \local_sic\context\contextinfo;
 ...
 * @param int $trackerid the current tracker
 * @return object|false issue object
...
function tracker_submitanissue(&$tracker, &$data) {
    ...
    if (isset($data->reportercontextid)){
        $issue->reportercontextid = $data->reportercontextid;
    } else {
        throw new coding_exception('Context not found');
        return false;
    }

    // SAMSUNG BEGIN - get assignee to ticket by provided context id
    if (!empty($data->assignedto)) {
        $issue->assignedto = 0 + $data->assignedto;
    } else if ($sic_assignee_id = tracker_resolve_sic_assignee($issue->reportercontextid)) {
        $issue->assignedto = $sic_assignee_id;
    ...
    if (empty($data->issueid)) {
        ...
        /*
         * SAMSUNG BEGIN - if assignee exists, register him as CCed(watcher) too
         */
...
function tracker_get_issues(&$tracker, $resolved, $screen, $sort, $limitfrom, $pagesize) {
    ...
    switch ($screen) {
        ...
        /*
         * SAMSUNG BEGIN - if someone tries to view all issues, handle this depending on capabilities of user
         */
        case 'browse': {
            global $context;
            // if manager, get all tickets
            if (has_capability('mod/tracker:manage', $context)) {
                break;
            }

            // if $USER - curator, then get context of his faculty and search by that
            $ctxs = \local_sic\Faculty::get_user_capability_contexts('enrol/sic:manage_faculty');

            $faculties = [];
            foreach($ctxs as $ctx) {
                $faculties[] = new \local_sic\Faculty($ctx->instanceid);
            }
            ...
            if ($ctxs) {
                ...
                list($insql, $inparams) = $DB->get_in_or_equal($ctxs_arr);
                $userclause = " AND reportercontextid $insql ";
...
function tracker_resolve_screen(&$tracker, &$cm, $getdefault = false) {
    ...
    if (!$getdefault) {
        $screen = optional_param('screen', 'browse', PARAM_ALPHA); // default: @$SESSION->tracker_current_screen
    }
    ...
    /*
     * SAMSUNG BEGIN - redirect to usual pages if user has no capability to access all issues (page 'browse')
     */
...
function tracker_can_edit(&$tracker, &$context, &$issue) {
    ...
    /*
     * SAMSUNG BEGIN -
     * no one except managers should be able to edit ticket after it's resolved
     * reporter shouldn't be able to edit tickets
     */
    /*
     * SAMSUNG END
     */
...
//    }
    ...
    if ($sic_resolvers && in_array($USER->id, $sic_resolvers)) {
        return true;
    }
...
/*
 * SAMSUNG BEGIN - set of custom functions for tracker
 */
/**
 * edits an issue in the current tracker
 * @param objectref $tracker the current tracker
 * @param objectref $data data object for update
 * @return false|object updated issue object if it exists already
 */
function tracker_editanissue(&$tracker, &$data) {
    global $DB, $USER;

    if (empty($data->issueid)) {
        throw new coding_exception("Invalid issue ID");
    }

    $cm = get_coursemodule_from_instance('tracker', $tracker->id);
    $context = context_module::instance($cm->id);

    if (!$issue = $DB->get_record('tracker_issue', ['id' => $data->issueid])) {
        return false;
    }

    // Setting data variables from existing issues to carry it onwards correctly in PHP script
    // "Summary" and "Description" form elemnts are frozen and don't get included in request
...
/** Gets resolvers for ticket according to SIC context of reporter
 * @param $issue
 * @return array|false
 * @throws coding_exception
 * @throws dml_exception
 */
function tracker_get_sic_resolvers($issue) {
    global $DB, $COURSE, $USER;

    if (empty($issue->reportercontextid)) {
        // some SQL queries from where the tracker_can_edit() is called, might not include this new field, so we
        // try getting full issue from DB
        $dbissue = $DB->get_record('tracker_issue', array('id' => $issue->id));
        if (empty($dbissue->reportercontextid)){
            return false;
        } else {
            $issue->reportercontextid = $dbissue->reportercontextid;
        }
    }

    $issuecontext = \context::instance_by_id($issue->reportercontextid);
//    $issuecontext = \context::instance_by_id(5668);
    $classname = get_class($issuecontext);

    $potentialresolvers = array();
    switch ($issuecontext->contextlevel){
        case \local_sic\context\contextinfo::CONTEXT_STUDENTGROUP:
            // TODO: get student group object

            // $studgroup = new \local_sic\StudentGroup($issuecontext->_instanceid);
            // $teacher = studgroup->getTeachers();

            // there MUST be only one teacher at studgroup
//            if (!empty($teacher)) {
//                $potentialresolvers[] = $teacher->id;
//            }

//            break;
        case \local_sic\context\contextinfo::CONTEXT_FACULTY:
            $faculty = new \local_sic\Faculty($issuecontext->instanceid);

            // there MUST be only one curator at faculty
            $curators = $faculty->getCurators();
            if (!empty($curators)) {
                $potentialresolvers[] = reset($curators)->userid;
            }
//            break;
        case CONTEXT_COURSE:
            $coursecontext = context_course::instance($COURSE->id);
            $role_id = get_config('local_sic', 'role_trackmanager');
            $role = $DB->get_record('role', ['id' => $role_id]);

            // there MUST be only one track curator at course
            $trackcurators = \get_users_from_role_on_context($role, $coursecontext);
            if (!empty($trackcurators)) {
                $potentialresolvers[] = reset($trackcurators)->userid;
            }
            break;
    }
    /* options:
    ...
    */
    ...
    if (empty($potentialresolvers)) {
        return false;
    } else {
        return $potentialresolvers;
    }
}

/**
 * Gets assignee for a ticket by ticket's reporter context id
 * @param int $contextid Reporter context id (possible context levels - StudentGroup, Faculty, Course)
 * @return false|int ID of user to be assigned
 */
function tracker_resolve_sic_assignee($contextid) {
    global $DB, $COURSE;
    $reportercontext = \context::instance_by_id($contextid);

    switch ($reportercontext->contextlevel) {
        case \local_sic\context\contextinfo::CONTEXT_STUDENTGROUP:
            // get student group object

            // $studgroup = new \local_sic\StudentGroup($reportercontext->_instanceid);
            // $teachers = studgroup->getTeachers();

            // there MUST be only one teacher at studgroup
//            if (!empty($teachers)) {
//                $assignee = reset($teachers)->userid;
//            }
            break;
        case \local_sic\context\contextinfo::CONTEXT_FACULTY:
            $faculty = new \local_sic\Faculty($reportercontext->instanceid);

            // there MUST be only one curator at faculty
            $curators = $faculty->getCurators();
            if (!empty($curators)) {
                $assignee = reset($curators)->userid;
            }
            break;
        case CONTEXT_COURSE:
            // get track curator
            ...
            $trackcurators = \get_users_from_role_on_context($role, $coursecontext);
            if (!empty($trackcurators)) {
                $assignee = reset($trackcurators)->userid;
            }
            break;
        default:
//            throw new coding_exception('Cannot resolve assignee');
    }

    return $assignee ? $assignee : false;

    // blueprint scheme:

    // verify that $USER is student ???

    // - resolve student status - self study or faculty

    // - if self study

    // --- find track curator (Andrey, Tatyana, Alexey) or head manager (Andrey)

    // - else if with faculty

    // --- try finding studgroup, if found

    // ----- find teacher of studgroup

    // --- else if no studgroup

    // ----- find curator of faculty
