...
class mod_tracker_renderer extends plugin_renderer_base {
    ...
    public function core_issue(&$issue, &$tracker) {
        ...
        $template->datereported = date('d.m.Y H:i:s', $issue->datereported); // userdate($issue->datereported);
     ...
     * SAMSUNG BEGIN
     */
    public function edit_link_raw($issue, $cm) {
        $out = '';

        $issueurl = new moodle_url('/mod/tracker/view.php');
        $editstr = get_string('edit');
        $out .= "<form method='post' action='{$issueurl}'>
                    <input type='hidden' name='id' value='{$cm->id}'>
                    <input type='hidden' name='view' value='view'>
                    <input type='hidden' name='screen' value='editanissue'>
                    <input type='hidden' name='issueid' value='{$issue->id}'>
                    <input type='submit' name='go_btn' value='{$editstr}>
                </form>";
        return $out;
    }
    /* 
    ...
    public function comments($comments, $initialviewmode, $addcommentlink = '') {
        ...
        if ($comments) {
            ...
            foreach ($comments as $comment) {
                ...
                $commenttpl->datecreated = date('d.m.Y H:i:s', $comment->datecreated); // userdate($comment->datecreated);
    ...
    public function history(&$tracker, $history, $statehistory, $initialviewmode) {
        ...
        if (!empty($history)) {
            foreach ($history as $owner) {
                ...
                $histtpl->date = date('d.m.Y H:i:s', $owner->timeassigned); // userdate($owner->timeassigned);
        ...
        if (!empty($statehistory)) {
            foreach ($statehistory as $state) {
                ...
                $histtpl->statedate = date('d.m.Y H:i:s', $state->timechange); // userdate($state->timechange);
    ...
    public function tabs($view, $screen, &$tracker, &$cm) {
        ...
        // SAMSUNG BEGIN - hide issues' counters in labels until their count is fixed accordingly or put elsewhere
        if ($screen == 'mytickets') {
...
//             $totalresolvedissues = $DB->count_records_select('tracker_issue', $select, array($tracker->id));
        ...
        // SAMSUNG BEGIN - hide issues' counters in labels until their count is fixed accordingly or put elsewhere
        // TODO: fix counters or render on pages instead of tabs
        ...
        // SAMSUNG BEGIN - hide reports until they are fully fixed
...
//                $rows[1][] = new tabobject('mywatches', $taburl, get_string('mywatches', 'tracker'));
    ...
    public function status_listform_part(&$tracker, &$cm, &$issue, &$context) {
        ...
        if (has_capability('mod/tracker:manage', $context)) {
            ...
            // SAMSUNG BEGIN - don't render quick solve button for issue list
    ...
    public function controls_listform_part(&$cm, &$issue, &$context) {
        ...
        $actions = '';
        /*
         * SAMSUNG BEGIN - DON'T RENDER TICKET BUTTONS:
         *      DELETE
         *      WATCH THIS TICKET
         *      PRIORITIES
         * ADD EDIT BUTTON FOR RESOLVERS
        ...
        if (!$DB->get_record('tracker_issuecc', array('trackerid' => $issue->trackerid, 'userid' => $USER->id, 'issueid' => $issue->id))) {
            $params = array('id' => $cm->id,
                            'view' => 'profile',
                            'screen' => $screen,
                            'issueid' => $issue->id,
        ...
        */
        ...
        if (tracker_can_edit($tracker, $context, $issue)) {
            // $example = self::edit_link($issue, $cm);
            // $editlink = self::edit_link_raw($issue, $cm);
            ...
            $alt = get_string('edit');
            $pix = $this->output->pix_icon('t/edit', $alt);
//            <span class="fa fa-edit icon tracker-issue-edit-handle" data-toggle="modal" data-target="#issuelist-edit-form" data-mode="status" data-context="{{contextid}}" id="issue-edit-status-handle-{{id}}"></span>
            $actions .= '&nbsp;<a href="'.$editurl.'" title="'.$alt.'">'.$pix.'</a>';
        }
    ...
    public function list_edit_form($args) {
        ...
        if (!$context) {
            throw new moodle_exception('Unknown context ID '.$args->ctx);
        }
        $systemcontext = context_system::instance();
    ...
    public function issuelist($issues, $totalcount, $cm, $tracker, $view, $screen, $resolved) {
        ...
        $template->haslistables = $tracker->haslistables ?? null;
